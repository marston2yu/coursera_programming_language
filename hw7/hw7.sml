...
(* CHANGE: Add a case for Shift expressions *)
	  | Shift(dx, dy, exp) => case eval_prog (exp, env) of
	  							...
	  							| _ => raise Impossible "bad call for shift: only work for values"
(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog (e) = 
	let
		fun ordered (x1, y1) (x2, y2) =
		 	if real_close(x1, x2) then (y1 < y2) else (x1 < x2)
	in
		case e of
		LineSegment(x1, y1, x2, y2) => if real_close_point (x1, y1) (x2, y2) 
		                               then Point(x1, y1) 
		                               else if ordered (x1, y1) (x2, y2) then e else LineSegment(x2, y2, x1, y1)
